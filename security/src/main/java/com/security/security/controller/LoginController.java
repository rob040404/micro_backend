...
public class LoginController {
    ...
    public ResponseEntity<?> login(@RequestBody(description = "DTO de petición",
            ...
            content = @Content(mediaType = "application/json",
                    schema = @Schema(implementation = AuthRequestDTO.class), examples = @ExampleObject(name = "ejemplo válido",
                    value = "{ \"email\": \"robert@robert.com\", \"password\": \"2313213\" }"
            )))
                                       @org.springframework.web.bind.annotation.RequestBody AuthRequestDTO authRequestDTO){


        log.trace("Entrando en /auth/login con {}, {}", authRequestDTO.getEmail(), authRequestDTO.getEmail());
